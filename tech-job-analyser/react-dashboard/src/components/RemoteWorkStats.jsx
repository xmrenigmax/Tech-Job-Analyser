...
const RemoteWorkStats = ({ data }) => {
  ...
  const remoteData = data && data.length > 0 ? data : [
    { index: 'Fully remote', count: 45 },
    { index: 'Hybrid', count: 35 },
    { index: 'Office', count: 20 }
  ];
  
  const total = remoteData.reduce((sum, item) => sum + item.count, 0);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {remoteData.map((item, index) => {
          ...
          return (
            <div key={item.index || index} className="text-center">
      ...
      <div className="space-y-3">
        ...
        {remoteData.map((item, index) => {
          const percentage = (item.count / total) * 100;
          return (
            <div key={item.index || index} className="flex items-center justify-between">
